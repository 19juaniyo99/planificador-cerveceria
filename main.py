...
# --- 4. SOLVER PRINCIPAL (Lógica Modular) ---
def resolver_modelo(datos: PlanificadorInput, relajado: bool = False):
    ...
    for d_idx, fecha in enumerate(fechas):
        ...
        for b in BANDAS:
            ...
            # REGLA APERTURA: Si es relajado bajamos a 1, si es estricto EXIGIMOS 2
            if b["es_apertura"]:
                fijos_apertura = [shifts[(n, d_idx, bid)] for n in nombres if mapa_emp[n].rol == "fijo"]
                min_apertura = 1 if relajado else 2
                model.Add(sum(fijos_apertura) >= min_apertura)
    ...
    for n in nombres:
        ...
        for d_idx in range(7):
            ...
            # Flexibilidad Diaria: 6h a 11h (para permitir encaje)
        ...
        # SI ES ESTRICTO: Mínimo = Lo que pidió el jefe. Máximo = +10h
        # SI ES RELAJADO (Diagnóstico): Permitimos -10h
        if relajado:
             model.Add(horas_sem >= emp.horas_objetivo - 10)
        else:
    ...
    solver.parameters.max_time_in_seconds = 30.0 # Más rápido para que si falla pruebe el relax
    st = solver.Solve(model)
    
    return st, solver, shifts, vacs, dias_str, nombres, mapa_emp

@app.post("/generar")
def generar(datos: PlanificadorInput):
    try:
        # 1. INTENTO ESTRICTO (Reglas del Jefe al 100%)
        st, solver, shifts, vacs, dias_str, nombres, mapa_emp = resolver_modelo(datos, relajado=False)

        if st not in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
            # 2. INTENTO DIAGNÓSTICO (Si falla el estricto, probamos el relajado para saber por qué)
            st_relax, _, _, _, _, _, _ = resolver_modelo(datos, relajado=True)
            
            motivo = "Desconocido (Conflicto complejo de turnos/descansos)"
            if st_relax in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
                motivo = "Conflicto de Horas: Tus empleados no suman suficientes horas para cubrir la demanda y los mínimos a la vez. O la regla de '2 en apertura' es imposible con los turnos disponibles."
            else:
                motivo = "Demanda Extrema: Faltan empleados. Incluso ignorando las horas mínimas es imposible cubrir el bar."

            return {
                "status": "IMPOSSIBLE", 
                "msg": f"No hay solución con reglas ESTRICTAS. Motivo probable: {motivo}. Revisa descansos o añade horas."
            }

        # Si hay solución estricta, la construimos
        res = {"dias": []}
        for d_idx, d_str in enumerate(dias_str):
